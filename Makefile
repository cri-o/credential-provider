...
$(BUILD_DIR)/$(PROJECT): $(BUILD_DIR) $(BUILD_FILES)
	CGO_ENABLED=$(CGO_ENABLED) GOARCH=$(GOARCH) GOOS=$(GOOS) $(GO) build -ldflags "-s -w -X github.com/cri-o/credential-provider/internal/pkg/config.RegistriesConfPath=$(REGISTRIES_CONF) -X github.com/cri-o/credential-provider/internal/pkg/version.buildDate=$(BUILD_DATE)" -o $(BUILD_DIR)/$(PROJECT) ./cmd/credential-provider
